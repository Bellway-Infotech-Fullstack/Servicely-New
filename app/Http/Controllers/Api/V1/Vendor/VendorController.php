...
class VendorController extends Controller
{
    public function get_profile(Request $request)
    {
        ...
        $restaurant = Helpers::restaurant_data_formatting($vendor, false);
        $restaurant['discount']=$vendor->discount;
    ...
    public function get_current_orders(Request $request)
    {
        ...
        ->where(function($query)use($vendor){
            if(config('order_confirmation_model') == 'restaurant' || $vendor->self_delivery_system)
            {
    ...
    public function update_order_status(Request $request)
    {
        ...
        if($request['status'] =="confirmed" && !$vendor->self_delivery_system && config('order_confirmation_model') == 'deliveryman' && $order->order_type != 'take_away')
        {
            return response()->json([
                'errors' => [
        ...
        if($request['status']=='delivered' && $order->order_type != 'take_away' && !$vendor->self_delivery_system)
        {
            return response()->json([
                'errors' => [
    ...
    public function get_notifications(Request $request){
        ...
        $notifications = Notification::active()->where(function($q) use($vendor){
            $q->whereNull('zone_id')->orWhere('zone_id', $vendor->zone_id);
    ...
    public function get_basic_campaigns(Request $request)
    {
        ...
        // $data = CampaignLogic::get_basic_campaigns($vendor->id, $request['limite'], $request['offset']);
        return response()->json($data, 200);
    }
    ...
    public function get_products(Request $request)
    {
        ...
        $paginator = Service::type($type)->where('vendor_id', $request['vendor']->id)->latest()->paginate($limit, ['*'], 'page', $offset);
        $data = [
            'total_size' => $paginator->total(),
            'limit' => $limit,
            'offset' => $offset,
